...
# ---------------------------
COINGECKO_API_KEY = "CG-chRgqiH9ab4zsFTm2Zvst82a"  # Replace with real Pro key if available
DEMO_KEY_PREFIX = "CG-"  # CoinGecko demo keys start with "CG-"

# Auto-switch to free API if using Demo key
if COINGECKO_API_KEY and COINGECKO_API_KEY.startswith(DEMO_KEY_PREFIX):
    st.warning("Demo API key detected → switching to free CoinGecko API")
...
def cg_coin_market_chart(coin_id: str, vs_currency: str, days: int) -> pd.DataFrame:
    ...
    df = prices.merge(volumes[["date","volume"]], on="date") \
               .merge(market_caps[["date","market_cap"]], on="date") \
...
st.title("Low-Cap Crypto Sector Crash Simulator & Backtest — Strategy 2")

st.markdown("""
This dashboard simulates risk proxies and backtests **Strategy 2**  
using CoinGecko API `/market_chart`.  

⚠️ Free API may have lower rate limits and slower responses for long histories.
""")
...
try:
    ...
    # ---------------------------
    work = df.copy()
    work["momentum"] = work["price"].pct_change(k_mom)
    work["log_price"] = np.log(work["price"])
    work["log_slope"] = work["log_price"].diff(k_mom)
    work["decay_flag"] = (work["log_slope"] < decay_threshold).astype(int)
    ...
    def make_positions(d: pd.DataFrame, entry_window: int, base_size: float) -> pd.Series:
        ...
        if liq_scale:
            ...
            liq_z = liq_z.clip(-2,2).fillna(0)
            size = base_size * (1 + 0.25 * liq_z)
        else:
            size = base_size
        pos = entry * size
    ...
    res = work.copy()
    res["pos_early"] = make_positions(work, early_window, 1.0)
    res["pos_mid"]   = make_positions(work, mid_window, 0.8)
    res["pos_late"]  = make_positions(work, late_window, 0.6)

    # ---------------------------
    # PnL and equity curves
    # ---------------------------
    def pnl_with_costs(pos: pd.Series, rets: pd.Series, fee_bps: float) -> pd.Series:
        pos_arr = pos.fillna(0).values
        prev = np.roll(pos_arr, 1); prev[0]=0
        turnover = np.abs(pos_arr - prev)
        tc = turnover * fee_bps/1e4
        return pd.Series(pos_arr * rets.fillna(0).values - tc, index=pos.index)
    ...
    def summarize(d: pd.DataFrame, equity: pd.DataFrame) -> pd.DataFrame:
        ...
        for key in ["pnl_early","pnl_mid","pnl_late","pnl_bh"]:
            r = d[key].fillna(0)
            cum = np.exp(r.cumsum().iloc[-1]) - 1
            vol = r.std() * np.sqrt(ann)
            sharpe = r.mean()*ann / (vol + 1e-12)
            curve = equity[key]
            mdd = (curve.cummax()/curve - 1).max()
            rows.append({"strategy": key.replace("pnl_",""), "cum_return": cum,
                         "ann_vol": vol, "sharpe": sharpe, "max_drawdown": mdd})
